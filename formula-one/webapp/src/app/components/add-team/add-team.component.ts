...
export class AddTeamComponent implements OnDestroy {
  ...
  private store = inject(Store<{ formulaOneItem: FormulaOneItem }>);
  ...
  constructor() {
    ...
    this.formGroup = new FormGroup({
      ...
      foundationYear: new FormControl(1950, [
        Validators.required,
        Validators.min(this.START_YEAR),
        Validators.max(2024),
      ]),
      championships: new FormControl(0, [
        Validators.required,
        Validators.min(0),
        Validators.max(100),
      ]),
    ...
    this.storeItem
      ...
      .subscribe((value: FormulaOneItem) => {
  ...
  fillExistingDataIntoForm(value: FormulaOneItem): void {
    ...
    this.formGroup.controls['entryFeeStatus'].setValue(
      value.entryFeeStatus === EntryFeeStatus.PAID
    );
    this.id = value.id!;
  }
  ...
  save(): void {
    const item: FormulaOneItem = {
      ...
      entryFeeStatus: this.formGroup.controls['entryFeeStatus'].value
        ? EntryFeeStatus.PAID
        : EntryFeeStatus.NOT_PAID,
    };
    if (this.editMode) {
      this.doUpdate(item);
      return;
  ...
  nameHasError(): boolean {
    return (
      ...
      this.formGroup.controls['name'].hasError('required')
    );
  }
  ...
  private checkAdded(value: FormulaOneItem): void {
    if (value && value.id) {
      this.messageService.add({
        ...
        detail: `${value.name} has been created.`,
      });
    }
  }
  ...
  private checkUpdated(value: FormulaOneItem): void {
    if (value && value.id) {
      this.messageService.add({
        ...
        detail: `${value.name} has been modified.`,
      });
    }
  }
